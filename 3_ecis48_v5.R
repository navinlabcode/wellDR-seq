...
pro_name <- "ecis48t"
pro_name1 <- "ecis48t_chip1"
pro_name2 <- "ecis48t_chip2"
disp_file1 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20220812_WDR_ECIS48T_Tube1/134808-2_Chip2_WellList.TXT")
disp_file2 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20220818_ECIS48T_Tube2/134731-2_Chip2_WellList.TXT")
...
myfolder_name_p1 <- "20221020_WDR_RNAlibs_reseq2_dropDR"
mysample_name_p1 <- "ECIS48T_Tube1_cDNA_Chip2_merge"
...
myfolder_name_p2 <- "20221104_WDR_RNAlibs_reseq5_MDA231_DNA_CUTTAG_AC"
mysample_name_p2 <- "ECIS48T_Tube2_Chip2_merge"
...
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
small_clst <- c("nc2","nc3","nc4","tc1","tc2","tc3")

chip_dna_path1 <- c("/volumes/USR2/wangkl/wscDR/20220817_WDR_ECIS48_tube1_chip2DR_BCIS31_Chip1R_LNA_TSO/data/waferDR_ECIS48T_tube1_chip2_DNA/waferDR_ECIS48_chip2-DNA_200/res_200_k")
chip_dna_path2 <- c("/volumes/USR2/wangkl/wscDR/20221206_WDR_DNAlib_reseq_ECIS48T_Tube2_Chip2/data/waferDR_ECIS48T_tube2_chip2_DNA/waferDR_ECIS48T_Tube2_Chip2-DNA_200/res_200_k")
...
cowplot::ggsave2(paste0("./figures/", pro_name_d, "_medicc2_ME_tree_merged_subclones_lumhr.pdf"), treeplt, width = 6, height = 2)
...
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
rna_celltype <- c("LumSec","LumHR","Fibro","ECs")
...
my_order <- c("c1",rev(lab[grepl("c[0-9]+", lab, perl = T)]),"c2","c4","c7")
...
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 18, height = 4*(length(my_clones)-1)/5, limitsize = F) 
...
bam_total_progenitor <- varbin_mtx_all_log2@colData %>% as.data.frame() %>% dplyr::filter(subclones3 == "c3") %>% pull(sample) %>% toupper()
bam_total_tumor <- varbin_mtx_all_log2@colData %>% as.data.frame() %>% dplyr::filter(subclones3 %in% paste0("c",5:6)) %>% pull(sample) %>% toupper()

#--bam files path
#chip1
met_path1 <- c("/volumes/USR2/wangkl/wscDR/20220817_WDR_ECIS48_tube1_chip2DR_BCIS31_Chip1R_LNA_TSO/data/waferDR_ECIS48T_tube1_chip2_DNA/waferDR_ECIS48_chip2-DNA_200/res_200_k/sort/")
#chip2
met_path2 <- c("/volumes/USR2/wangkl/wscDR/20221206_WDR_DNAlib_reseq_ECIS48T_Tube2_Chip2/data/waferDR_ECIS48T_tube2_chip2_DNA/waferDR_ECIS48T_Tube2_Chip2-DNA_200/res_200_k/sort/")
...
for (i in c("normal", "normalHR", "progenitor", "tumor")) {
  ...
  bam_p1 <- paste0(met_path1, my_file[str_detect(my_file, "TUBE1_CHIP2")] %>% str_replace("TUBE1_CHIP2", "Tube1_Chip2"),".sort.markdup.bam") %>% as.data.frame()
  bam_p2 <- paste0(met_path2, my_file[str_detect(my_file, "TUBE2_CHIP2")] %>% str_replace("TUBE2_CHIP2", "Tube2_Chip2"),".sort.markdup.bam") %>% as.data.frame()
...
epi_freq <- celltype_freq[c(1:2),]
sum(sapply(epi_freq, sum))
epi_freq2 <- epi_freq[1, 7]/768
