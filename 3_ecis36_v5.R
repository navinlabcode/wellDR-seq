...
# detach("package:copykit", unload = TRUE)
...
disp_file1 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20210830_low_dNTPs_high_dCTPs_ECIS36T_BCMHBCA14R/133468-2_Chip1_WellList.TXT")
disp_file2 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20210907_WDR_ECIS25T_ECIS36T_Chip2/133440-2_Chip2_WellList.TXT")
...
myfolder_name_p1 <- "20221005_WDR_RNAlibs_reseq"
mysample_name_p1 <- "ECIS36T_Chip1_RNA_merge"
...
myfolder_name_p2 <- "20210912_WDR_ECIS25TDFR_ECIS36T-2R"
mysample_name_p2 <- "ECIS36T_Chip2_RNA"
...
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
small_clst <- c("c2","c15")
chip_dna_path1 <- c("/volumes/USR2/wangkl/wscDR/20221105_WDR_DNAlib_reseq_ECIS36_tube1_chip1/data/waferDR_ECIS36_tube1_chip1_DNA_merged/waferDR_ECIS36_tube1_chip1-DNA_merged_200/res_200_k")
chip_dna_path2 <- c("/volumes/USR2/wangkl/wscDR/20220918_WDR_ECIS36_chip2_DNA_1M_dropDR5pct_gel/data/waferDR_ECIS36_chip2_DNA_1M/waferDR_ECIS36_chip2-DNA_1M_200/res_200_k")
...
obj_rna <- readRDS(paste0("/volumes/USR1/ruiye/10x/2020ANALYSIS/WDR/WDR_Paper/DCIS_RNA_Object/SCTV2_no_integration/", toupper(pro_name), "_RNA_original.sct"))
...
# dim <- c(5,8,10,15,20,30)
# pgroup <- c("orig.ident")
# plists <- dim_res_finder2(res, dim, obj_rna, pgroup)
# plist_ru <- plot_grid(plotlist =plists[4:5],ncol = 1)
# p_list<-plot_grid(plists[[3]],plist_ru, ncol = 1)
# cowplot::ggsave2(paste0(c("./figures/"), pro_name_r, c("_tSNE_test.pdf")), plists[[1]], width = 6*(length(res)+1), height = 5*length(dim))
# cowplot::ggsave2(paste0(c("./figures/"), pro_name_r, c("_UMAP_test.pdf")), plists[[2]], width = 6*(length(res)+1), height = 5*length(dim))
...
# sel_dim <- 20
# sel_res <- 0.8
# obj_rna_tmp <- RunUMAP(obj_rna, dims = 1:sel_dim)
# obj_rna_tmp <- FindNeighbors(obj_rna_tmp, dims = 1:sel_dim) %>% FindClusters(resolution = sel_res)
# DimPlot(obj_rna_tmp, reduction = "umap")
# 
# obj_rna_tmp.markers <- FindAllMarkers(obj_rna_tmp, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25, assay = "RNA")
# obj_rna_tmp.markers %>% dplyr::group_by(cluster) %>% top_n(2, avg_log2FC)
# top10 <- obj_rna_tmp.markers %>% dplyr::group_by(cluster) %>% top_n(10, avg_log2FC)
# DoHeatmap(obj_rna_tmp, features = top10$gene)
...
write.csv(dna_seg_rna_df, 
          paste0("./metrics/",pro_name, "_genotype_seg_vs_sudo_bulk_expr_seg_Statistic_WithMorethan_", min_cell_num, "_Cells.csv"))
# dna_seg_rna_df <- read.csv(paste0("./metrics/",pro_name, "_genotype_seg_vs_sudo_bulk_expr_seg_Statistic_WithMorethan_", min_cell_num, "_Cells.csv"),
# row.names = 1)
...
ggplot(data_long, aes(fill = category, y = count, x = subclone_comb)) + 
  geom_bar(position = "stack", stat = "identity") +
  theme_minimal() +
  labs(x = "Subclone Combination", y = "Count", title = "Stacked Bar Plot for Subclone Combinations")

#---venn diagram---
library("VennDiagram")
library("RColorBrewer")
p1 <- draw.pairwise.venn(area1 = 24+100, area2 = 31+100, cross.area = 100, fill = brewer.pal(3, "Set2")[1:2], 
                         category = c("CNA diff", "RNA diff"), lty = "blank", ext.line.lty = "dashed")
...
# dna_seg_rna_df <- read.csv(paste0("./metrics/",pro_name, "_genotype_seg_vs_sudo_bulk_expr_seg_Statistic_WithMorethan_", min_cell_num, "_Cells_cutoff_test.csv"),
# row.names = 1)
...
cowplot::ggsave2(paste0("./figures/", pro_name, "_genotype_seg_vs_sudo_bulk_expr_seg_Statistic_WithMorethan_",min_cell_num,"_Cells_cutoff_test.pdf"), 
                 p1, width = 8, height = 5)
...
p1 <- ggplot(data_long485, aes(x = subclone_comb, y = count, fill = CNA_status, label= count)) +
  geom_bar(stat = "identity", position = "stack") + 
...
cowplot::ggsave2(paste0("./figures/", pro_name, "_DE_gene_in_clonal_subclonal_regions_WithMorethan_",min_cell_num,"_Cells_log2fc_0.485.pdf"), 
                 p1, width = 3.5, height = 3)

data_long485 %>% dplyr::group_by(subclone_comb) %>% dplyr::summarise(total_count = sum(count), clonal_count = sum(count[CNA_status == "clonal"])) %>% 
  mutate(clonal_pct = clonal_count/total_count*100)
...
for (i in 1:nrow(combn_df)) {
  ...
  write.csv(de.markers, 
            paste0("./metrics/de_marker_genetic_dis/",pro_name_r, "_default_de_markers_",super_clone1, "_vs_",super_clone2,"_downsampled_",subsample_min,".csv"))
  # de.markers <- read.csv(
  # paste0("./metrics/de_marker_genetic_dis/",pro_name_r, "_default_de_markers_",super_clone1, "_vs_",super_clone2,"_downsampled_",subsample_min,".csv"), 
  # row.names = 1)
  # #--0.263: 1.2X. 0.485: 1.4x. 0.678: 1.6x; 0.848: 1.8x. 1: 2X
...
#---DE analysis of subclones 7 versus 13 ---#####
obj_rna <- readRDS(paste0("/volumes/USR1/ruiye/10x/2020ANALYSIS/WDR/WDR_Paper/DCIS_RNA_Object/SCTV2_no_integration/", toupper(pro_name), "_RNA.sct.rds"))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
obj_rna2 <- obj_rna
...
de_markers_rank <- de.markers$avg_log2FC
names(de_markers_rank) <- rownames(de.markers)

plists <- pathways_detector4(de_markers_rank, genelist_gsea_hall, genelist_gsea_c5_bp, genelist_gsea_kegg, genelist_gsea_rectome, top_n = 10)
#---Save GSEA results--
pathway_list <- c("gsea_hall","gsea_c5_bp","gsea_kegg","gsea_reactome")# "gsea_c2","gsea_all",
j <- 1
for (m in pathway_list) {
  fwrite(plists[[3]][[j]], file=paste0("./metrics/DE_pathways/de_markers_", super_name1, "_vs_", super_name2, "_fgsea_results.txt"), sep="\t", sep2=c("", " ", ""))
  j <- j+1
}
cowplot::ggsave2(paste0("./figures/", pro_name_r, "de_markers_",super_name1, "_vs_", super_name2, "_fgsea_test.pdf"), plists[[1]], width = 70, height = 80, limitsize = F)
cowplot::ggsave2(paste0("./figures/", pro_name_r, "de_markers_",super_name1, "_vs_", super_name2, "_fgsea_test_top10.pdf"), plists[[2]], width = 50, height = 10, limitsize = F)

#----test add modual score---
#-reactome can't do this because there are too many pathways--
library(data.table)
library(knitr)
library(GSEABase)
library(genefilter)
...
obj_rna3 <- obj_rna2
obj_rna3 <- AddModuleScore(obj_rna3, features = genelist_gsea_hall, name = "HALLMARK")
colnames(obj_rna3@meta.data)[grep("HALLMARK", colnames(obj_rna3@meta.data))] <- names(genelist_gsea_hall)
obj_rna3 <- AddModuleScore(obj_rna3, features = genelist_gsea_kegg, name = "KEGG")
colnames(obj_rna3@meta.data)[grep("KEGG", colnames(obj_rna3@meta.data))] <- names(genelist_gsea_kegg)

df_hall <- obj_rna3@meta.data %>% dplyr::select(c("temp",starts_with("HALLMARK"))) %>% dplyr::filter(temp != "others") %>% 
  as.data.frame() %>% rownames_to_column("id")
df_kegg <- obj_rna3@meta.data %>% dplyr::select(c("temp",starts_with("KEGG"))) %>% dplyr::filter(temp != "others") %>% 
  as.data.frame() %>% rownames_to_column("id")

df_hall_long <- df_hall %>% pivot_longer(cols = -c(id, temp), names_to = "hallmark", values_to = "value")
df_kegg_long <- df_kegg %>% pivot_longer(cols = -c(id, temp), names_to = "kegg", values_to = "value")

library(ggpubr)
p1 <- ggboxplot(df_hall_long, x = "temp", y = "value", color = "value", add = "jitter", facet.by = "hallmark", scales=("free_y")) + stat_compare_means()
p2 <- ggboxplot(df_kegg_long, x = "temp", y = "value", color = "value", add = "jitter", facet.by = "kegg", scales=("free_y")) + stat_compare_means()

cowplot::ggsave2(paste0("./figures/",pro_name_r, "_",super_name1, "_vs_", super_name2, "_test_hallmark.pdf"), p1, width = 20, height = 20, limitsize = F)
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_",super_name1, "_vs_", super_name2, "_test_kegg.pdf"), p2, width = 40, height = 40, limitsize = F)


hall_pvalue <- df_hall_long %>% dplyr::group_by(hallmark) %>% dplyr::summarise(p_value = wilcox.test(value ~ temp)$p.value) %>% mutate(p_adj = p.adjust(p_value))
kegg_pvalue <- df_kegg_long %>% dplyr::group_by(kegg) %>% dplyr::summarise(p_value = wilcox.test(value ~ temp)$p.value) %>% mutate(p_adj = p.adjust(p_value))

hall_pvalue_sig <- hall_pvalue %>% filter(p_adj < 0.05)
kegg_pvalue_sig <- kegg_pvalue %>% filter(p_adj < 0.05)

df_hall_long_sig <- df_hall_long %>% dplyr::filter(hallmark %in% hall_pvalue_sig$hallmark) %>% mutate(hallmark2 = stringr::str_replace(hallmark, "HALLMARK_", ""))
df_kegg_long_sig <- df_kegg_long %>% dplyr::filter(kegg %in% kegg_pvalue_sig$kegg) %>% mutate(kegg2 = stringr::str_replace(kegg, "KEGG_", ""))

p3 <- ggboxplot(df_hall_long_sig, x = "temp", y = "value", color = "value", add = "jitter", facet.by = "hallmark2", scales=("free_y"), ncol=4) + stat_compare_means()
p4 <- ggboxplot(df_kegg_long_sig, x = "temp", y = "value", color = "value", add = "jitter", facet.by = "kegg2", scales=("free_y"), ncol=4) + stat_compare_means()

cowplot::ggsave2(paste0("./figures/",pro_name_r, "_",super_name1, "_vs_", super_name2, "_test_hallmark_padj_sig.pdf"), p3, 
                 width = 12, height = nrow(hall_pvalue_sig)/4*3.5, limitsize = F)
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_",super_name1, "_vs_", super_name2, "_test_kegg_padj_sig.pdf"), p4, 
                 width = 12, height = nrow(hall_pvalue_sig)/4*3.5, limitsize = F)
...
gene_bin_sel <- gene_bin %>% dplyr::filter(gene %in% rownames(rbind(de_genes_up, de_genes_down))) %>% 
  mutate(col=ifelse(dos_state[pos] == "sCNA", "#EB008B", "#424451"))

ha_bottom_cs = columnAnnotation(df = dos_state_df, col = list(dos_state=c("sCNA"="#EB008B", "cCNA"="#424451")), 
...
gene_bin_sel_sample <- gene_bin_sel %>% dplyr::mutate(my_dos = plyr::mapvalues(col, from = c("#424451","#EB008B"), to = c("cCNA", "sCNA"))) 

df <- gene_bin_sel_sample
p1 <- ggplot(df, aes(x=my_dos, fill=my_dos)) + geom_bar() + theme_classic() + 
  scale_fill_manual(values=c("sCNA"="#EB008B", "cCNA"="#424451")) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
#------Make DNA-RNA co-line plots (c7 vs c13)---#########
obj_rna <- readRDS(paste0("/volumes/USR1/ruiye/10x/2020ANALYSIS/WDR/WDR_Paper/DCIS_RNA_Object/SCTV2_no_integration/", toupper(pro_name), "_RNA.sct.rds"))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
...
plot_s <- list()
j<-1
#---checked chr18-22, X, due to the chr size and gene expression number, it's can't do the smoonth. 
for (i in paste0("chr",c(1:17),":")) {
  # i <- "chrX:"
...
grouped_median %>% filter(abs(median_diff) > 0.02)


#---DE analysis of subclones 11 versus 13 ---#####
obj_rna <- readRDS(paste0("/volumes/USR1/ruiye/10x/2020ANALYSIS/WDR/WDR_Paper/DCIS_RNA_Object/SCTV2_no_integration/", toupper(pro_name), "_RNA.sct.rds"))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
obj_rna2 <- obj_rna
...
#------Make DNA-RNA co-line plots (c11 vs c13)---#########
obj_rna <- readRDS(paste0("/volumes/USR1/ruiye/10x/2020ANALYSIS/WDR/WDR_Paper/DCIS_RNA_Object/SCTV2_no_integration/", toupper(pro_name), "_RNA.sct.rds"))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
...
plot_s <- list()
j<-1
#---checked chr17-22, X, due to the chr size and gene expression number, it's can't do the smoonth. 
for (i in paste0("chr",c(1:16,"X"),":")) {
  # i <- "chr16:"
  dna_rna_merged_cna_exp_target <- dna_rna_merged_cna_exp %>% mutate(rowname = as.integer(rowname)) %>% dplyr::filter(stringr::str_detect(chr_pos, i))
  dna_rna_merged_cna_exp_target <- dna_rna_merged_cna_exp_target %>% mutate(mean_scaled_exp_clone1 = ifelse(mean_scaled_exp_clone1>0.05, mean_scaled_exp_clone1, NA), 
                                                                            mean_scaled_exp_clone2 = ifelse(mean_scaled_exp_clone2>0.05, mean_scaled_exp_clone2, NA))
...
cowplot::ggsave2(paste0("./figures/", pro_name, "_dna_rna_line_plot_",super_clone1, "_vs_",super_clone2, "_chr1_16.pdf"), p_list, width = 25, height = 16)
