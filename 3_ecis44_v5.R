...
pro_name <- "ecis44t"
pro_name1 <- "ecis44t_chip1"
pro_name2 <- "ecis44t_chip2"
pro_name3 <- "ecis44t_chip3"
pro_name4 <- "ecis44t_chip4"
pro_name5 <- "ecis44t_chip5"

disp_file1 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20211221_ECIS44T/134741-2_Chip2_WellList.TXT")
disp_file2 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20211229_ECIS44T_Tube2/134671-2_Chip1_WellList.TXT")
disp_file3 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20211229_ECIS44T_Tube2/134810-2_Chip2_WellList.TXT")
disp_file4 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/2022040_ECIS44T_Tube4/133725-2_Chip1_WellList.TXT")
disp_file5 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20220104_ECIS44T_Tube3/134805-2_Chip2_WellList.TXT")
...
# file.copy(disp_file3, paste0("./metrics/dispense_files/", pro_name3, "_", basename(disp_file3)))
# file.copy(disp_file4, paste0("./metrics/dispense_files/", pro_name4, "_", basename(disp_file4)))
# file.copy(disp_file5, paste0("./metrics/dispense_files/", pro_name5, "_", basename(disp_file5)))
...
disp3 <- read.delim(paste0("./metrics/dispense_files/", pro_name3, "_", basename(disp_file3)))
disp4 <- read.delim(paste0("./metrics/dispense_files/", pro_name4, "_", basename(disp_file4)))
disp5 <- read.delim(paste0("./metrics/dispense_files/", pro_name5, "_", basename(disp_file5)))
...
myfolder_name_p1 <- "20221025_WDR_RNAlibs_reseq4"
mysample_name_p1 <- "ECIS44T_Tube1_Chip2_merge"
...
myfolder_name_p2 <- "20211231_WDR_ECIS44-72EDTA_chip1R_60PCR1-chip2DR"
mysample_name_p2 <- "ECIS44T_Tube2_cDNA_Chip1"
...
myfolder_name_p3 <- "20221025_WDR_RNAlibs_reseq4"
mysample_name_p3 <- "ECIS44T_Tube2_Chip2_merge"
...
myfolder_name_p4 <- "20221025_WDR_RNAlibs_reseq4"
mysample_name_p4 <- "ECIS44T_Tube4_Chip1_merge"
raw_path_r4 <- c(paste0("/volumes/USR2/wangkl/wscDR/", myfolder_name_p4, "/data/", 
                        mysample_name_p4, "_exact_hg19_star_smart_2/star_2.7.5/",
                        mysample_name_p4, "_exact_hg19_star_smart_2_Solo.out/GeneFull/filtered_nodup"))
myfolder_name_p5 <- "20221025_WDR_RNAlibs_reseq4"
mysample_name_p5 <- "ECIS44T_Tube3_Chip2_merge"
raw_path_r5 <- c(paste0("/volumes/USR2/wangkl/wscDR/", myfolder_name_p5, "/data/", 
                        mysample_name_p5, "_exact_hg19_star_smart_2/star_2.7.5/",
                        mysample_name_p5, "_exact_hg19_star_smart_2_Solo.out/GeneFull/filtered_nodup"))
...
raw_path5 <- paste0("./map_seg_output/", pro_name5)
...
#---t5--
varbin_mtx5 <- readVarbin(raw_path5, remove_Y = TRUE)
varbin_mtx5 # 809
filt_cells_names_tmp5 <- rownames(varbin_mtx5@colData) %>% as.data.frame() %>% mutate(cell = stringr::str_extract(., "c\\d+")) %>% dplyr::rename(cell_name = ".")
filt_cells_names_dis5 <- disp5 %>% dplyr::filter(!(Sample %in% c("Neg Ctrl", "Pos Ctrl"))) %>%  dplyr::select(1,2,4) %>% mutate(row_col=paste(Row, Col, sep = "_")) %>% 
  dplyr::select(3:4) %>% left_join(wafer_match_list, by = c("row_col" = "coor")) %>% mutate(cell2 = tolower(Cell)) %>% 
  inner_join(filt_cells_names_tmp5, by = c("cell2" = "cell")) %>% pull(cell_name)

varbin_mtx_all5 <- varbin_mtx5[,filt_cells_names_dis5]
varbin_mtx_all5@colData # 559
...
varbin_mtx_all5@colData$experiment <- pro_name5

varbin_mtx_all <- cbind(varbin_mtx_all1, varbin_mtx_all2,varbin_mtx_all3, varbin_mtx_all4,varbin_mtx_all5)
...
names(exp_col) <- c(pro_name1, pro_name2, pro_name3, pro_name4, pro_name5)
#-----header---
...
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
small_clst <- c("nc2","tc1","tc2","tc3","tc4","tc5")

chip_dna_path1 <- c("/volumes/USR2/wangkl/wscDR/20211223_WDR_ECIS44_chip1R_chip2DR/data/waferDR_ECIS44_chip1_DNA/waferDR_ECIS44_chip2-DNA_200/res_200_k")
chip_dna_path2 <- c("/volumes/USR2/wangkl/wscDR/20221128_WDR_DNAlib_reseq_ECIS44T_Tube2_Chip1/data/waferDR_ECIS44T_Tube2_Chip1_DNA/waferDR_ECIS44T_Tube2_Chip1-DNA_200/res_200_k")
chip_dna_path3 <- c("/volumes/USR2/wangkl/wscDR/20211231_WDR_ECIS44-72EDTA_chip1R_60PCR1-chip2DR/data/waferDR_ECIS44_chip2_DNA/waferDR_ECIS44_chip2-DNA_200/res_200_k")
chip_dna_path4 <- c("/volumes/USR2/wangkl/wscDR/20220405_WDR_ECIS44_tube4_chip1DR_chip2R/data/waferDR_ECIS44_tube4_chip1_DNA/waferDR_ECIS44_tube4_chip1-DNA_200/res_200_k")
chip_dna_path5 <- c("/volumes/USR2/wangkl/wscDR/20221129_WDR_DNAlib_reseq_ECIS44T_Tube3_Chip2/data/waferDR_ECIS44T_tube3_chip2_DNA/waferDR_ECIS44T_Tube3_Chip2-DNA_200/res_200_k")
...
for (i in small_clst) {
  ...
  p_list <- apply(cell_mtx, 1, FUN = function(x){
    ...
    } else if (stringr::str_detect(x["experiment"], "chip5")){
      mypath <- chip_dna_path5
    }
...
#----------MEDICC2 ME Tree---lumsec----######
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
## make medicc input matrix
#--other clusters--
other_clst <- c("c5","c8","c9")
dip_clst <- c("c1")
medicc_input <- cbind(varbin_mtx_all_log2@rowRanges %>% dplyr::as_tibble() %>% dplyr::select(seqnames, start, end), varbin_mtx_all_log2@consensus) %>%
  mutate(diploid=2) %>%       ## assign diploid cell CN profile to be used as root in the medicc tree
  dplyr::rename(chrom=seqnames) %>% gather('sample_id', 'CN', -chrom, -start, -end) %>% dplyr::select(sample_id,chrom, everything()) %>% 
...
medicc_input <- medicc_input %>%
  ...
  mutate(
    CN = case_when(
      row_number() %in% 1:10 ~ CN[11],  # For the first three rows, use the CN value of the fourth row
      row_number() %in% (n() - 9):n() ~ CN[n() - 10],  # For the last two rows, use the CN value of the third row from the bottom
      TRUE ~ CN  # For all other rows, keep the original CN value
    )
  )

dir.create(paste0("metrics/medicc_files_lumhr/", pro_name_d),recursive = T)
write_tsv(medicc_input, file = paste0("metrics/medicc_files_lumhr/", pro_name_d, "/",pro_name_d, c("_lumsec_medicc2_input.tsv")))
# medicc_input <- read_tsv(file = paste0("metrics/medicc_files_lumhr/", pro_name_d, "/",pro_name_d, c("_lumsec_medicc2_input.tsv")))

mywd <- getwd()
#----go to terminal and run medicc2---
# conda activate medicc_env
# medicc2 -a CN --total-copy-numbers -j 40 -vv input_path/*_lumsec_medicc2_input.tsv output_path
# eg: medicc2 -a CN --total-copy-numbers -j 40 -vv ./*_lumsec_medicc2_input.tsv ./
tsv_out_path <- paste0(mywd, "/metrics/medicc_files_lumhr/", pro_name_d, "/")
#---ME tree, need two diploid cell populations to make diploid branch on the most left side.
medic_dis <- read_tsv(paste0(tsv_out_path, pro_name_d, "_lumsec_medicc2_input_pairwise_distances.tsv"))
medic_dis2 <- medic_dis %>% as.data.frame() %>% column_to_rownames(var="sample_id") %>% as.dist()
me_tree <- ape::fastme.bal(medic_dis2)
me_tree <- ape::root.phylo(me_tree, outgroup = "diploid", resolve.root = TRUE)
me_tree <- ape::drop.tip(me_tree, tip = dip_clst)
...
cowplot::ggsave2(paste0("./figures/", pro_name_d, "_medicc2_ME_tree_merged_subclones_lumsec.pdf"), treeplt, width = 6, height = 2.5)

####----RNA data integration--------#######
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","LumHR","LumSec","MyoEpi","ECs","Tumor","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","Mito","Peri")
rna.cluster.col <- c("#D89000","#FF62BC","#00BD5F","#9590FF","#F8766D","#54278f","#BF4000","#3969AC","grey90","grey60","grey70","grey80","#CAB2D6","#fb9a99")
...
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
rna_celltype <- c("LumSec","LumHR", "Tumor","Fibro","Peri","ECs","MyoEpi")
...
pdf(paste0(c("./figures/"),pro_name_r, c("_Seurat_top10_ComplexHeatmap.pdf")), height = 13, width = 5.5)
...
#---tumor cells
df_hall <- obj_rna3@meta.data %>% dplyr::filter(subclone3 == "c9") %>% dplyr::select(c("subclone3",ends_with("ModScore"))) %>% 
...
p1 <- ggplot(df_hall_long, aes(x=ModScore, y = value)) + geom_boxplot() + geom_jitter(position = position_jitter(0.1)) + theme_classic() + 
  theme(axis.title.x = element_blank())
p1
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_tumor_epi_modual_score.pdf"), p1, width = 3, height = 3)
...
p_list1 <- plot_grid(plotlist = plist, ncol = 5)
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 18, height = 3.2*(length(my_clones)-1)/5, limitsize = F) 
...
#---DNA subclone frequency----####
# install.packages("ggbreak")
library(ggbreak)
# install.packages("aplot")
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
sub_freq <- varbin_mtx_all_log2@colData$subclones3 %>% table() %>% as.data.frame() %>% mutate(pct = Freq/2980*100)


p1 <- ggplot(sub_freq, aes(x = `.`, y = pct, fill = `.`)) + geom_col(orientation="x") + 
  scale_fill_manual(values = new_pal[1:nrow(sub_freq)])

p2 <- p1 + scale_y_break(c(20, 70)) + scale_y_break(c(2, 13)) + 
  geom_text(aes(label = round(pct, 2)), vjust = 1) + 
  xlab(NULL) + ylab(NULL) + theme_classic()

cowplot::ggsave2(paste0("./figures/", pro_name_d, "_subclone_freq.pdf"), p2, width = 4, height = 3)
...
epi_freq <- celltype_freq[c(1:3,7),]
