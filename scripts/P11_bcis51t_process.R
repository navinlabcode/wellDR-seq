...
pro_name <- "bcis51t"
pro_name1 <- "bcis51t_chip1"
pro_name2 <- "bcis51t_chip2"
...
nrow(varbin_mtx_all_log_knn2@colData) # 2647
nrow(varbin_mtx_normal@colData) # 247
nrow(varbin_mtx_tumor@colData) # 2400
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.09, spread = 3, n_neighbors = 7))
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_tumor_log@assays@data$segment_ratios)), 
                                    metric = "manhattan", min_dist = 0.2, spread = 1, n_neighbors = 23))
umap_data2 <- umap_data %>% dplyr::rename("UMAP_1" = "X1", "UMAP_2" = "X2")
...
to_clone1 <- c("c4","c4","c7")
from_clone1 <- c("c5","c6","c8")
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","Tumor4","LumSec","MyoEpi","Peri","Cancer","Tumor2","Tumor3","RNA_miss","Unknown","Unknown2","Unknown3","VasEndo", "others")
...
name_dna$rna_bc2 <- gsub("BCIS51T_","",name_dna$rna_bc2)
...
# RNA data UMAP
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","Tumor4","Normal","MyoEpi","Peri","Tumor","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","VasEndo", "CAFs")
rna.cluster.col <- c("#D89000","#FF62BC","#00BD5F","#9590FF","#F8766D","#54278f","#BF4000","#3969AC","grey90","grey60","grey70","grey80","#CAB2D6","#fca311")
names(rna.cluster.col) <- rna.cluster.ids

obj_rna <- readRDS("objects/BCIS51T_SCTV2_combined.rds")
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
...
saveRDS(obj_rna,paste0("./objects/", pro_name_r, c("_final_merged_integerCN.rds")))
...
for (i in my_clones[1:length(my_clones)]) {
  ...
  mut_cell_df <- obj_rna_newid_tmp@meta.data[,c("RNA_seurat_clusters","subclone3")] %>% table() %>% as.data.frame() %>% dplyr::filter(Freq >0 & Freq <3)
  mut_cell_chr <-  mut_cell_df %>% dplyr::filter(subclone3 == i) %>% pull(RNA_seurat_clusters) %>% as.character()
  obj_rna_newid_tmp@meta.data <- obj_rna_newid_tmp@meta.data %>% mutate(subclone4 = ifelse((subclone4 == i & RNA_seurat_clusters %in% mut_cell_chr), "unsel", subclone4))
