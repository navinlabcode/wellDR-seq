...
pro_name <- "bcis106t"
pro_name1 <- "bcis106t_chip1"
pro_name2 <- "bcis106t_chip2"
...
nrow(varbin_mtx_all_log_knn2@colData) # 3105
nrow(varbin_mtx_normal@colData) # 1038
nrow(varbin_mtx_tumor@colData) # 2067
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.05, spread = 2, n_neighbors = 3))
...
mtx <- varbin_mtx_normal_log3@consensus %>%
  dplyr::filter(c1>=3 | c1<=1 )

ht_mtx <- as.data.frame(varbin_mtx_normal_log3@consensus)
ht_mtx[bin_name,] <- 2
varbin_mtx_normal_log3@consensus <- ht_mtx


tmp <- as.data.frame(varbin_mtx_normal_log@assays@data$segment_ratios)
tmp[rownames(centromere_mtx),] <- 1
varbin_mtx_normal_log2@assays@data$segment_ratios <- tmp
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_tumor_log@assays@data$segment_ratios)), 
                                    metric = "manhattan", min_dist = 1, spread = 3, n_neighbors = near_nb))
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","Tumor4","LumSec","MyoEpi","Peri","Tumor","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","Mito", "CAFs")
...
obj_rna <- readRDS("objects/BCIS106T_SCTV2_combined.rds")

varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_new.rds")))
#varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
rna_celltype <- c("Tumor","Tumor2","Fibro","Peri")
...
name_dna$rna_bc2 <- gsub("BCIS106T_","",name_dna$rna_bc2)
...
obj_rna_newid@meta.data$subclone3 <- factor(obj_rna_newid@meta.data$subclone3,levels = c(paste0("c",1:10),"unsigned"))
obj_rna_newid<-obj_rna
#rna_celltype <- c("Tumor","Tumor2","Fibro","Tumor3","Peri","Tumor4")
