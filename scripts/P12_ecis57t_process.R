...
pro_name <- "ecis57t"
pro_name1 <- "ecis57t_chip1"
pro_name2 <- "ecis57t_chip2"
...
#varbin_mtx_all_log <- readRDS(file = paste0("./wdr_analysis/objects/", pro_name_d, c("_without_filtered_all_copykit_rnameta.rds")))
...
nrow(varbin_mtx_all_log_knn2@colData) # 2337
nrow(varbin_mtx_normal@colData) # 721
nrow(varbin_mtx_tumor@colData) # 1616

saveRDS(varbin_mtx_normal, file = paste0("./objects/", pro_name_d, c("_knn_normal_copykit_before_rename.rds")))
#varbin_mtx_normal <- readRDS("D:/dna-rna/wdr_analysis/objects/bcis66t_dna_knn_normal_copykit_before_rename.rds")

saveRDS(varbin_mtx_tumor, file = paste0("objects/", pro_name_d, c("_knn_tumor_copykit_before_rename.rds")))
#varbin_mtx_tumor <- readRDS("D:/dna-rna/wdr_analysis/objects/bcis66t_dna_knn_tumor_copykit_before_rename.rds")
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.05, spread = 1, n_neighbors = 7))
...
## heatmap with c0
mtx_srt <- as.data.frame(varbin_mtx_normal_log@colData) %>% mutate(subclones = as.character(subclones)) %>% 
  mutate(subclones = factor(subclones, levels = paste0("c", 0:(length(unique(varbin_mtx_normal_log@colData$subclones))-1)))) %>% 
  arrange(subclones)
...
anno_mtx$subclones <- factor(anno_mtx$subclones, levels = paste0("c", 0:(length(unique(anno_mtx$subclones))-1)))

clst_col <- new_pal_tmp[1:length(unique(anno_mtx$subclones))]
names(clst_col) <- paste0("c", 0:(length(unique(anno_mtx$subclones))-1))
...
pdf(paste0("./figures/", pro_name_d, "_knn_normal_complexHeatmap_DR_with_c0.pdf"), height = 10, width = 10)
...
css <- as.data.frame(varbin_mtx_normal_log3@consensus) %>%
  dplyr::filter(c5 >2)

bin <- as.data.frame(varbin_mtx_normal_log3@rowRanges)

con_tmp <- varbin_mtx_normal_log3@consensus
con_tmp0 <- con_tmp %>% rownames_to_column()
ref_tem <- varbin_mtx_normal_log3@rowRanges %>% as.data.frame() %>% rownames_to_column() %>% dplyr::select(1:2)

clst_comp <- data.frame()
for (i in 1:(ncol(con_tmp)-1)) {
  for (j in (i+1):ncol(con_tmp)) {
    diff_loc <- con_tmp0$rowname[con_tmp[,i] != con_tmp[,j]]
    diff_loc <-  gsub("V","",diff_loc)
...
pdf(paste0("./figures/", pro_name_d, "_merged_integer_complexHeatmap_DR.pdf"), height = 15, width = 10)
Heatmap(as.matrix(ht_mtx), cluster_columns = FALSE, border = TRUE, cluster_rows = F, show_row_dend = F,
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","LumHR","LumSec","MyoEpi","VasEndo","Tumor","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","Tcell", "CAFs")
...
obj_rna <- readRDS("objects/ECIS57T_SCTV2_combined.rds")  
#varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_new.rds")))

rna_celltype <- c("Tumor","Tumor2","Fibro","VasEndo","Tcell","MyoEpi")
...
name_dna$rna_bc2 <- gsub("ECIS57T_","",name_dna$rna_bc2)
...
#varbin_mtx_all_log2 <- readRDS(paste0("./wdr_analysis/objects/", pro_name_d, c("_final_merged_integerCN_copykit_RNA_meta.rds")))
...
saveRDS(obj_rna,paste0("objects/", pro_name_r, c("_final_merged_integerCN_copykit_rna_meta.rds")))
...
obj_rna_newid@meta.data$subclone3 <- factor(obj_rna_newid@meta.data$subclone3,c(paste0("c",1:13),"unsigned"))
plist <- list()
j <- 1
...
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 18, height = 25, limitsize = F) 
