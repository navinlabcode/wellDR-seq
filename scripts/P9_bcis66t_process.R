...
pro_name <- "bcis66t"
pro_name1 <- "bcis66t_chip1"
pro_name2 <- "bcis66t_chip2"
...
nrow(varbin_mtx_all_log_knn2@colData) # 2551
nrow(varbin_mtx_normal@colData) # 1860
nrow(varbin_mtx_tumor@colData) # 691
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.1, spread = 3, n_neighbors = 25))
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_tumor_log@assays@data$segment_ratios)), 
                                    metric = "manhattan", min_dist = 0.05, spread = 3, n_neighbors = 5))
...
to_clone1 <- c("c1","c7","c7","c7","c7")
from_clone1 <- c("c8","c9","c10","c11","c12")
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","LumHR","LumSec","MyoEpi","Peri","Tumor","Tumor2","LumHR2","RNA_miss","others","Unknown2","Unknown3","VasEndo", "Unknown1")
...
obj_rna <- readRDS("objects/BCIS66T_SCTV2_combined.rds")

varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_new.rds")))
#varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))

rna_celltype <- c("LumHR","LumHR2","Tumor","Tumor2","MyoEpi","Fibro","Peri","LumSec","VasEndo")
...
n_col <- 6
pdf(paste0("./figures/", pro_name, "typical_marker_feature_plot_violin_tumor.pdf"), width = 3*n_col, height = 3*ceiling(length(typical_marker)/n_col))
p1 <- VlnPlot(object = obj_rna, feature = typical_marker, ncol = n_col)
dev.off()

p2 <- DimPlot(obj_rna, reduction = "umap")
cowplot::ggsave2(paste0("./figures/", pro_name, "_umap.pdf"), p2, width = 5, height = 5)
...
name_dna$rna_bc2 <- gsub("BCIS66T_","",name_dna$rna_bc2)
...
DNA_rna_meta <- obj_rna@meta.data %>% rownames_to_column() %>% mutate(rowname = toupper(rowname)) %>% 
  ...
  dplyr::select(c("nCount_RNA", "nFeature_RNA", "RNA_dispense", "RNA_S5XX", "RNA_percent.mt", "RNA_nCount_SCT", "RNA_nFeature_SCT", "RNA_seurat_clusters")) %>% 
  dplyr::mutate(cell_type = plyr::mapvalues(RNA_seurat_clusters, from = levels(RNA_seurat_clusters), to = rna_celltype))
...
Idents(obj_rna) <- "seurat_clusters" 
names(typical_marker) <- paste0(names(typical_marker),"_ModScore")
obj_rna3 <- subset(obj_rna,idents=0)
obj_rna3 <- AddModuleScore(obj_rna3, features = typical_marker, name = "modscore")
colnames(obj_rna3@meta.data)[grep("modscore", colnames(obj_rna3@meta.data))] <- names(typical_marker)

#---tumor cells
df_hall <- obj_rna3@meta.data %>% 
  dplyr::select(ends_with("ModScore")) %>% 
  as.data.frame() %>% rownames_to_column("id")
df_hall_long <- df_hall %>% pivot_longer(cols = -c(id), names_to = "ModScore", values_to = "value")
...
all_markers <- read.csv("/sibcb1/wangkailelab1/pengjinyu/wscDR/wscDR/pre_load_data/allcell_markers.csv")
cluster <- unique(all_markers$cluster)
typical_marker <- list()
for (i in cluster){
  #i <- "Basal"
  tmp <- all_markers %>%
    ...
    head(20)
  names(tmp) <- i
  typical_marker <- append(typical_marker, tmp)
}

obj_rna4 <- subset(obj_rna,idents=c("LumHR","Unknown1"))
de.markers <- FindMarkers(obj_rna4, ident.1 = "LumHR", ident.2 = "Unknown1")
...
# rna_col <- rna.cluster.col[names(rna.cluster.col) %in% unique(anno_mtx$rna_clst)]
meta_con <- varbin_mtx_all_log2@colData %>% as.data.frame() %>% dplyr::select(subclones3, RNA_cell_type) %>% 
  dplyr::group_by(subclones3, RNA_cell_type) %>% dplyr::summarise(n=n()) %>% spread(RNA_cell_type, n) %>% dplyr::select(-`<NA>`)
...
#-----annotated genes---
gene_bin <- read.table("./pre_load_data/hg19_gene_binpos_map.tsv", header = 1) #cannot find 
...
obj_rna_newid@meta.data$subclone3 <- factor(obj_rna_newid@meta.data$subclone3,c(paste0("c",1:3),"unsigned"))
plist <- list()
j <- 1
...
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 18, height = 10, limitsize = F) 
...
pgrid_1 <- plot_grid(plotlist = plist, ncol = 2)
cowplot::ggsave2(paste0("./figures/", pro_name_d, "_modulescore_by_clones.pdf"), pgrid_1, width = 15, height = 10 ,limitsize = FALSE)
