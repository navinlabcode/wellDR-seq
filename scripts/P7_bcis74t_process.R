...
pro_name <- "bcis74t"
pro_name1 <- "bcis74t_chip1"
pro_name2 <- "bcis74t_chip2"
...
varbin_mtx_all_log <- readRDS(file = paste0("./objects/", pro_name_d, c("_without_filtered_all_copykit_rnameta.rds")))


#------Knn smooth-----#######
source("./0_wdr_functions.R")
metadata(varbin_mtx_all_log)$genome <-  "hg19"
varbin_mtx_all_log_knn <- knnSmooth_kaile(varbin_mtx_all_log,k=1)
...
nrow(varbin_mtx_all_log_knn2@colData) # 2709
nrow(varbin_mtx_normal@colData) # 1327 1088 1121
nrow(varbin_mtx_tumor@colData) # 1382  1621 1588

saveRDS(varbin_mtx_normal, file = paste0("./objects/", pro_name_d, c("_knn_normal_copykit_before_rename.rds")))
#varbin_mtx_normal <- readRDS(paste0("./objects/", pro_name_d, c("_knn_normal_copykit_before_rename_with_res_3.rds")))

saveRDS(varbin_mtx_tumor, file = paste0("objects/", pro_name_d, c("_knn_tumor_copykit_before_rename.rds")))
#varbin_mtx_tumor <- readRDS(paste0("objects/", pro_name_d, c("_knn_tumor_copykit_before_rename_with_res_3.rds")))
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.01, spread = 3, n_neighbors = 5))
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_tumor_log@assays@data$segment_ratios)), 
                                    metric = "manhattan", min_dist = 0.05, spread = 2, n_neighbors = near_nb))
...
if("c0" %in% names(table(varbin_mtx_tumor_log@colData$subclones))){
  ...
  clone_num
  kept_clones <- as.data.frame(clone_num) %>% dplyr::filter(Var1 != "c0") %>% dplyr::filter(Freq > 3) 
  #kept_clones <- as.data.frame(clone_num) %>% dplyr::filter(Freq > 3)
  varbin_mtx_tumor_log <- varbin_mtx_tumor_log[, subclones %in% kept_clones$Var1]
  varbin_mtx_tumor_log@colData #1398
}else{
  varbin_mtx_tumor_log@colData$subclones <- factor(varbin_mtx_tumor_log@colData$subclones,
                                                   levels = paste0("c", 1:(length(table(subclones)))))
}
...
ha_row2=rowAnnotation(df = anno_mtx, col = list(subclones=clst_col, experiment=exp_col),
                      show_annotation_name = F, simple_anno_size = unit(0.7, "cm"))
pdf(paste0("./figures/", pro_name_d, "_knn_tumor_complexHeatmap_before_remove_doublets_DR.pdf"), height = 20, width = 10)
Heatmap(as.matrix(ht_mtx), cluster_columns = FALSE, border = TRUE, cluster_rows = F, show_row_dend = F,
        row_split = anno_mtx$subclones,
        name = "scheatmap", show_row_names = F, show_column_names = F, row_title = paste0(nrow(ht_mtx), " single cells"),
        # column_title = paste0(pro_name, "_low_",low_cutoff, "_up_", up_cutoff, "_cell_pct_", cell_cutoff,"_",neu_cutoff),
...
cowplot::ggsave2(paste0("./figures/", pro_name_d, "_scquantum_ploidy_confidence_by_subclone1.pdf"), p2, 
                 width = length(unique(varbin_mtx_tumor_log2@colData$subclones)), height = 2,limitsize = FALSE)
...
bad_sub <- c("c9","c22","c23","c26","c28","c38","c39")
#----------Remove bad clusters----#######
...
to_clone1 <-   c("c1","c1", "c1", "c1", "c1", "c1", "c1", "c1","c1", "c1", "c1", "c1", "c6","c10","c12","c16","c16","c16","c16","c25")
from_clone1 <- c("c2","c4","c20","c21","c30","c31","c32","c33","c34","c35","c36","c37","c7","c11","c13","c17","c18","c24","c29","c27")
...
gene_bin_sel <- gene_bin %>% dplyr::filter(gene %in% c("AKT3","FHIT","PIK3CA","FGFR4","CDKN1A","ESR1","FGFR1","CCNE2","MYC","CDKN2A",
                                                       "FOXA1","CDH5","WWOX","TP53","ERBB2","BRCA1","FOXK2"))
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","Tumor","LumSec","MyoEpi","VasEndo","Tumor4","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","Mito")
rna.cluster.col <- c("#D89000","#da88dc","#00BD5F","#9590FF","#F8766D","#54278f","#BF4000","#3969AC","grey90","grey60","grey70","grey80","#CAB2D1")
names(rna.cluster.col) <- rna.cluster.ids

obj_rna <- readRDS("./objects/BCIS74T_SCTV2_combined.rds")

varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_new.rds")))
#varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
rna_celltype <- c("Tumor","Tumor2","Fibro","Tumor3","Tumor4","VasEndo","MyoEpi")
...
n_col <- 6
pdf(paste0("./figures/", pro_name, "typical_marker_feature_plot_violin.pdf"), width = 3*n_col, height = 3*ceiling(length(typical_marker)/n_col))
VlnPlot(object = obj_rna, feature = typical_marker, ncol = n_col)
dev.off()

p1 <- DimPlot(obj_rna, reduction = "umap")
cowplot::ggsave2(paste0("./figures/", pro_name, "umap.pdf"), p1, width = 5, height = 5)
...
name_dna$rna_bc2 <- gsub("BCIS74T_","",name_dna$rna_bc2)
...
DNA_rna_meta <- obj_rna@meta.data %>% rownames_to_column() %>% mutate(rowname = toupper(rowname)) %>% 
  dplyr::left_join(name_dna, ., by = c("rna_bc2" = "rowname")) %>% 
  dplyr::select(c("nCount_RNA", "nFeature_RNA", "RNA_dispense", "RNA_S5XX", "RNA_percent.mt", "RNA_nCount_SCT", "RNA_nFeature_SCT", 
                  "RNA_seurat_clusters")) %>% 
  dplyr::mutate(cell_type = plyr::mapvalues(RNA_seurat_clusters, from = levels(obj_rna@meta.data$RNA_seurat_clusters), to = rna_celltype))

varbin_mtx_all_log2@colData <- cbind(varbin_mtx_all_log2@colData, DNA_rna_meta)
saveRDS(varbin_mtx_all_log2, file = paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_RNA_meta_new.rds")))
...
obj_rna <- readRDS("./objects/BCIS74T_SCTV2_combined.rds")
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
name_dna <- varbin_mtx_all_log2@colData %>% as.data.frame() %>% dplyr::select(c("sample", "experiment","subclones3")) %>% 
  mutate(newname = toupper(stringr::str_extract(sample, "c\\d+"))) %>% left_join(wafer_match_list, by = c("newname" = "Cell")) %>% 
  mutate(rna_bc2 = paste(toupper(experiment), RNA_Barcode, sep = "_"))
name_dna$rna_bc2 <- gsub("BCIS74T_","",name_dna$rna_bc2)
RNA_dna_subclone3 <- obj_rna@meta.data %>% rownames_to_column() %>% mutate(rowname = toupper(rowname)) %>% 
  dplyr::left_join( .,name_dna, by = c("rowname" = "rna_bc2")) %>%
  dplyr::select(subclones3) %>%
  mutate_if(is.factor, as.character) %>%
  mutate_all(~ replace(., is.na(.), "unsigned"))

obj_rna@meta.data$subclone3 <- RNA_dna_subclone3$subclones3
#saveRDS(obj_rna,paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit_rna_meta.rds")))
...
pdf(paste0(c("./figures/"),pro_name_r, c("_Seurat_top10_ComplexHeatmap.pdf")), height = 15, width = 10)
...
df_hall_others <- obj_rna3@meta.data %>% dplyr::filter(subclone3 %in% paste0("c", c(2:10,12))) %>% dplyr::select(c("subclone3","LumHR_ModScore")) %>% 
  as.data.frame() %>% rownames_to_column("id") %>% mutate(newlabel= "others")

df_hall_c11 <- obj_rna3@meta.data %>% dplyr::filter(subclone3 %in% paste0("c", 11)) %>% dplyr::select(c("subclone3","LumHR_ModScore")) %>% 
  as.data.frame() %>% rownames_to_column("id") %>% mutate(newlabel= "c11")
df_hall_c13 <- obj_rna3@meta.data %>% dplyr::filter(subclone3 %in% paste0("c", 13)) %>% dplyr::select(c("subclone3","LumHR_ModScore")) %>% 
  as.data.frame() %>% rownames_to_column("id") %>% mutate(newlabel= "c13")
df_hall_long <- rbind(df_hall_others,df_hall_c11,df_hall_c12)

wilcox.test(df_hall_c11$LumHR_ModScore,df_hall_c13$LumHR_ModScore)
wilcox.test(df_hall_c13$LumHR_ModScore,df_hall_others$LumHR_ModScore)

df_hall_long$newlabel <- factor(df_hall_long$newlabel,levels = c("c13","c11","others"))
p1 <- ggplot(df_hall_long, aes(x=newlabel, y = LumHR_ModScore, color=newlabel)) + geom_boxplot() + geom_jitter(position = position_jitter(0.1)) + theme_bw()+ 
  theme(panel.grid  = element_blank()) + scale_color_manual(values= c("#D4A2D9" ,"#C1395E","#54278f")) 
p1<-p1+stat_compare_means(method = "wilcox.test", label = "p.format", comparisons = list(c("c13", "c11"), c("c13", "others"), c("c11", "others")))


cowplot::ggsave2(paste0("./figures/bcis74t_progenitor_epi_modual_score.pdf"), p1, width = 3, height = 3)

#---map DNA clusters to RNA UMAP----#####
plist <- list()
j <- 1
my_clones <- unique(obj_rna_newid@meta.data$subclone3)
...
p_list1 <- plot_grid(plotlist = plist, ncol = 3)
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 15, height = 20, limitsize = F) 
...
obj_rna <- readRDS(paste0("objects/", pro_name_r, c("_final_merged_integerCN.rds")))
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_final_merged_integerCN_copykit.rds")))
gene_bin <- read.table("./pre_load_data/hg19_gene_binpos_map.tsv", header = 1)
obj_rna2 <- obj_rna
