...
pro_name <- "bcis70t"
pro_name1 <- "bcis70t_chip1"
pro_name2 <- "bcis70t_chip2"
...
nrow(varbin_mtx_all_log_knn2@colData) # 3069
nrow(varbin_mtx_normal@colData) # 1327
nrow(varbin_mtx_tumor@colData) # 1742
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_normal_log@assays@data$segment_ratios)),
                                    metric = "manhattan", min_dist = 0.2, spread = 1, n_neighbors = 25))
...
umap_data <-  data.frame(uwot::umap(log(t(varbin_mtx_tumor_log@assays@data$segment_ratios)), 
                                    metric = "manhattan", min_dist = 0.05, spread = 3, n_neighbors = near_nb))
...
to_clone1 <- c("c11","c11","c11")
from_clone1 <- c("c12","c13","c15")
...
pro_name_r <- paste0(pro_name, "_rna")
rna.cluster.ids<-c("Fibro","LumHR","LumSec","LumHR2","Peri","Cancer","Tumor2","Tumor3","RNA_miss","Unknown1","Unknown2","Unknown3","VasEndo", "CAFs")
...
obj_rna <- readRDS("objects/BCIS70T_RNA.rds")
varbin_mtx_all_log2 <- readRDS(paste0("objects/", pro_name_d, c("_merged_integerCN_copykit_new.rds")))


rna_celltype <- c("LumSec","LumHR","Fibro","Peri","VasEndo","Cancer")
...
name_dna$rna_bc2 <- gsub("BCIS70T_","",name_dna$rna_bc2)
...
# varbin_mtx_all_log2@colData$cell_type <- gsub("Tumor","Cancer",varbin_mtx_all_log2@colData$cell_type)
# varbin_mtx_all_log2@colData$cell_type <- gsub("CAFs","Fibro",varbin_mtx_all_log2@colData$cell_type)
# varbin_mtx_all_log2@colData$cell_type <- gsub("ECs","VasEndo",varbin_mtx_all_log2@colData$cell_type)
...
rna_celltype <- c("LumHR","Cancer","Fibro","LumSec","Peri","VasEndo")
...
pdf(paste0(c("./figures/"),pro_name_r, c("_Seurat_top10_ComplexHeatmap.pdf")), height = 20, width = 10)
...
obj_rna_newid@meta.data$subclone3 <- factor(obj_rna_newid@meta.data$subclone3, c(paste0("c",1:9),"unsigned"))
plist <- list()
j <- 1
...
cowplot::ggsave2(paste0("./figures/", pro_name_r, "_DNA_subclone_on_RNA_UMAP.pdf"), p_list1, width = 18, height = 20, limitsize = F) 
...
p1
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_tumor_epi_modual_score.pdf"), p1, width = 6, height = 3)


pgrid_1 <- plot_grid(plotlist = plist, ncol = 4)
cowplot::ggsave2(paste0("./figures/", pro_name_d, "_tumor_epi_modual_score_per_clones.pdf"), pgrid_1, width = 15, height = 10 ,limitsize = FALSE)
