...
disp_file1 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20210907_WDR_ECIS25T_ECIS36T_Chip2/133437-2_Chip1_WellList.TXT")
disp_file2 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20210917_ECIS25T_Tube2/133459-2_Chip1_WellList.TXT")
disp_file3 <- c("/volumes/lab/users/wet_lab/instruments/wafergen/Rui_WDR/filter_files/20210917_ECIS25T_Tube2/133460-2_Chip2_WellList.TXT")
...
myfolder_name_p1 <- "20210912_WDR_ECIS25TDFR_ECIS36T-2R"
mysample_name_p1 <- "ECIS25T_Chip1_RNA"
...
mysample_name_p2 <- "ECIS25T_Tube2_cDNA_Chip1_merge"
...
mysample_name_p3 <- "ECIS25T_Tube2_cDNA_Chip2_merge"
...
#---add RNA_index info---
mtx2@meta.data <- mtx2@meta.data %>% rownames_to_column() %>% 
  mutate(N7xx_bc = stringr::str_sub(rowname, -16, -9), S5xx_bc = stringr::str_sub(rowname, -8, -1)) %>%
  column_to_rownames()
...
chip_dna_path1 <- c("/volumes/USR2/wangkl/wscDR/20210912_WDR_ECIS25TDFR_ECIS36T-2R/data/waferDR_ECIS25T_DNA/waferDR_ECIS25T_DNA_200/res_200_k")
chip_dna_path2 <- c("/volumes/USR2/wangkl/wscDR/20221123_WDR_DNAlib_reseq_ECIS25T_Tube2_Chip1/data/waferDR_ECIS25T_tube2_chip1_DNA/waferDR_ECIS25_tube2_chip1-DNA_200/res_200_k")
chip_dna_path3 <- c("/volumes/USR2/wangkl/wscDR/20221124_WDR_DNAlib_reseq_ECIS25T_Tube2_Chip2/data/waferDR_ECIS25_tube2_chip2_merged/waferDR_ECIS25_tube2_chip2-DNA_merged_200/res_200_k")
...
lab45_uniq_ext <- c("CKDN2C","AKT3","CKS1B","RUVBL1","C8orf4","FGFR1","BAG4","MTDH","MYC","PAK1","CDK4","MDM2", "FHIT","NEDD9","FOXK2","CCNE2",
                    "PLA2G10","GRB7","RPS6KB1","PPM1D","CCNE1","YWHAB","ZNF217","AURKA","PTK6","PPP2R2A",
                    "CCND1","NCOA3","TP53","CDKN1A","GADD45A","MDM2","RB1","CDKN2A","CDKN2C","ESR1","SHC1",
                    "PGR","PIK3CA","PTEN","BCL2","GATA3","FGFR4","EGFR","ERBB2","EMSY","BRCA1","FOXA1","BRCA2","NF2")
#---from: Chromosome 16 tumor-suppressor genes in breast cancer --- https://doi.org/10.1002/gcc.20318
chr16_tumor_suppressor <- c("E2F4","CTCF","TERF2","FBXL8","LRRC29","CDH5","CDH16","CDH3","CDH11","ATBF1","BBC1","FANCA","GAS11","CPNE7","ZFP276",
                            "CBFA2T3","CYBA","WWOX","TERF21P","SIAH1") 
chr16_tumor_suppressor2 <- c("E2F4","CTCF","TERF2","FBXL8","CDH5","CDH16","CDH3","CDH11","GAS11","CPNE7","ZFP276","CBFA2T3","WWOX","TERF21P") 

gene_bin_sel <- gene_bin %>% dplyr::filter(gene %in% lab45_uniq_ext)
ha_bottom_cs = columnAnnotation(clonal_state = anno_mark(at=gene_bin_sel$pos, labels = gene_bin_sel$gene, 
                                                         side = "bottom", labels_gp = gpar(fontsize = 6)))
...
#---progenitor cells
df_hall <- obj_rna3@meta.data %>% dplyr::filter(subclone3 == "c2") %>% dplyr::select(c("subclone3",ends_with("ModScore"))) %>% 
...
p1
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_c2_epi_modual_score.pdf"), p1, width = 3, height = 3)

#---tumor cells
p1 <- ggplot(df_hall_long, aes(x=ModScore, y = value)) + geom_boxplot() + geom_jitter(position = position_jitter(0.1)) + theme_classic() + 
  theme(axis.title.x = element_blank())
p1
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_all_tumor_epi_modual_score.pdf"), p1, width = 3, height = 3)


#----tumor cells and progenitor cells
df_hall_tumor_prog <- obj_rna3@meta.data %>% dplyr::filter(!(subclone3 %in% c("c1","unassigned"))) %>% 
  dplyr::select(c("subclone3",ends_with("ModScore"))) %>% 
  as.data.frame() %>% rownames_to_column("id")
df_hall_tumor_prog_long <- df_hall_tumor_prog %>% pivot_longer(cols = -c(id, subclone3), names_to = "ModScore", values_to = "value")
df_hall_tumor_prog_long2 <- df_hall_tumor_prog_long %>% mutate(subclone_temp=ifelse(subclone3=="c2", "c2", "tumor"))
p1 <- ggplot(df_hall_tumor_prog_long2, aes(x=ModScore, y = value, color=ModScore)) + geom_boxplot() + geom_jitter(position = position_jitter(0.2), size = 0.1) + 
  scale_color_manual(values= c("#9590FF","#FF62BC","#00BD5F")) + #theme_classic() + 
  theme(axis.title.x = element_blank()) + facet_wrap(~subclone_temp, nrow = 1, strip.position = "top")
p1
g <- ggplot_gtable(ggplot_build(p1))
strip_top <- which(grepl('strip-t', g$layout$name))
fills <- c(new_pal[2],"#54278f")
k <- 1
for (i in strip_top) {
...
grid.draw(g)
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_all_tumor_progenitor_epi_modual_score.pdf"), g, width = 4, height = 2)


#---tumor cells clone by clone
df_hall_tumor <- obj_rna3@meta.data %>% dplyr::filter(!(subclone3 %in% c("c1","c2","unassigned"))) %>% 
  dplyr::select(c("subclone3",ends_with("ModScore"))) %>% 
  as.data.frame() %>% rownames_to_column("id")
df_hall_tumor_long <- df_hall_tumor %>% pivot_longer(cols = -c(id, subclone3), names_to = "ModScore", values_to = "value")

p1 <- ggplot(df_hall_tumor_long, aes(x=ModScore, y = value, color=ModScore)) + geom_boxplot() + geom_jitter(position = position_jitter(0.01)) + 
  scale_color_manual(values= c("#9590FF","#FF62BC","#00BD5F")) + #theme_classic() + 
  theme(axis.title.x = element_blank()) + facet_wrap(~subclone3, nrow = 2, strip.position = "top")

g <- ggplot_gtable(ggplot_build(p1))
strip_top <- which(grepl('strip-t', g$layout$name))
fills <- c(new_pal[13:22],new_pal[3:12])
k <- 1
for (i in strip_top) {
  j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
  g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
...
grid.draw(g)
cowplot::ggsave2(paste0("./figures/",pro_name_r, "_all_tumor_by_subclones_epi_modual_score.pdf"), g, width = 10, height = 3)
...
for (i in my_clones[1:(length(my_clones)-1)]) {
  ...
  mut_cell_df <- obj_rna_newid_tmp@meta.data[,c("merged_cluster","subclone3")] %>% table() %>% as.data.frame() %>% dplyr::filter(Freq >0 & Freq <3)
  mut_cell_chr <-  mut_cell_df %>% dplyr::filter(subclone3 == i) %>% pull(merged_cluster) %>% as.character()
  obj_rna_newid_tmp@meta.data <- obj_rna_newid_tmp@meta.data %>% mutate(subclone4 = ifelse((subclone4 == i & merged_cluster %in% mut_cell_chr), "unsel", subclone4))
...
#--bam files path
#chip1
met_path1 <- c("/volumes/USR2/wangkl/wscDR/20210912_WDR_ECIS25TDFR_ECIS36T-2R/data/waferDR_ECIS25T_DNA/waferDR_ECIS25T_DNA_200/res_200_k/sort/")
#chip2
met_path2 <- c("/volumes/USR2/wangkl/wscDR/20221123_WDR_DNAlib_reseq_ECIS25T_Tube2_Chip1/data/waferDR_ECIS25T_tube2_chip1_DNA/waferDR_ECIS25_tube2_chip1-DNA_200/res_200_k/sort/")
#chip3
met_path3 <- c("/volumes/USR2/wangkl/wscDR/20221124_WDR_DNAlib_reseq_ECIS25T_Tube2_Chip2/data/waferDR_ECIS25_tube2_chip2_merged/waferDR_ECIS25_tube2_chip2-DNA_merged_200/res_200_k/sort/")
